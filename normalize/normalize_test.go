...
//
// Ported from github.com/luckyPipewrench/pipelock,
// internal/normalize/normalize_test.go, at commit
// 41e5c57051866d43c5d87cb4e7d5a927cfe3f95f (Apache-2.0).
//
// Changed from upstream: the ForDLP tests (TestForDLP_Parity,
// TestForDLP_NegativeSquared, TestForDLP_ExoticWhitespaceEvasion, BenchmarkForDLP)
// were dropped along with the ForDLP pipeline itself -- see normalize.go. The
// ordering constraint they covered, that StripExoticWhitespace must run before
// NFKC, is preserved directly in TestStripExoticWhitespace_MustRunBeforeNFKC.
// Benchmarks were rewritten off testing.Loop (Go 1.24+) to keep the go 1.22 floor.
...
// StripExoticWhitespace's doc comment asserts, and that the dropped ForDLP parity
// test used to cover incidentally.
//
// NFKC compatibility-decomposes NBSP and the other exotic spaces to a plain ASCII
// space. Once that has happened, StripExoticWhitespace can no longer tell an
// evasion splitter from legitimate spacing, and the splitter survives as a literal
// space in the middle of the token it was inserted to break. Strip first and the
// token is restored; strip second and it is not.
//
// A future refactor that reorders these two steps in any pipeline will fail here,
...
func TestStripExoticWhitespace_MustRunBeforeNFKC(t *testing.T) {
	...
	// The wrong order. Asserted explicitly so the failure mode is documented in
	// code rather than folklore: NFKC turns the NBSP into a space that then
	// survives the strip, because ASCII whitespace is legitimate content.
