...
//
// Ported from github.com/luckyPipewrench/pipelock, internal/normalize/normalize.go,
// at commit 41e5c57051866d43c5d87cb4e7d5a927cfe3f95f (Apache-2.0). See NOTICE at
// the repository root, and docs/pipelock-port.md for why.
//
// Changed from upstream: package doc rewritten for airlock, and the ForDLP
// pipeline was dropped as out of scope -- airlock inspects text going into a
// prompt or coming back out of a reply, not data leaving over the network. The
// primitives ForDLP composed (StripControlChars, StripExoticWhitespace) are kept;
// only the DLP-branded pipeline is gone.
//
// The non-ASCII runes below are written as \u escapes, as upstream wrote them.
// That is deliberate and worth preserving: the characters this package defends
// against are invisible or visually identical to ASCII, so spelling them literally
// would make the tables impossible to review.
...
// # Choosing a pipeline
//
// Three pipelines compose the primitives below. Pick by what the text is:
//
...
//     than dropping them. Use it where dropping one would fuse two tokens into
//     one, so that "rm<ZWSP>-rf" stays "rm -rf" instead of becoming "rm-rf".
...
//
// [FoldVowels] is not a pipeline but an extra pass applied on top of one, for a
...
// lookalikes commonly used in homoglyph attacks.
//
// This table is hand-rolled, NOT Unicode's confusables data, and it is not
// exhaustive - it targets characters that appear in English-language injection
// phrases. A homoglyph missing from this table passes through unfolded, so a gap
// here is a real detector gap. If you find one, add the rune and add a test.
...
// rather than folding them to a space the way Whitespace does.
//
// Use it where a token cannot legitimately contain whitespace at all, so exotic
// whitespace inside it is evasion rather than content -- an attacker splitting a
// wrap fence sentinel with an NBSP, say. Folding to a space would leave a literal
// space mid-token and still defeat the match; removing it restores the token.
//
// Must run BEFORE NFKC, because NFKC compatibility-decomposes NBSP, U+3000, and
// U+2000-200A to ASCII space, after which they are indistinguishable from
// legitimate spacing and can no longer be removed.
//
// ASCII whitespace (' ', '\t', '\n', '\r') is preserved: legitimate content uses
// it. The rune set matches Whitespace() exactly, so the two functions share one
// mental model of what counts as stego whitespace.
...
//
// The signal is advisory and exposure-only. Pattern matching is unaffected by
// it, because ForMatching already strips combining marks via StripCombiningMarks.
// It exists so a caller can report that text arrived obfuscated, independently of
// whether any pattern actually matched -- the evidence is gone by the time the
// match runs, so it has to be measured on the raw input first.
...
// injection evasion.
//
// It is lossy in a way the other primitives are not: it rewrites legitimate digits
// too, so "sha256" becomes "shazsb". Apply it only where digits carry no meaning
// worth keeping (tool metadata), never to text you intend to compare literally.
...
// strips the whitespace control chars (\t, \n, \r), so it destroys line structure.
// Use it only where any control character is evasion rather than content, which is
// true of MCP tool metadata but not of freetext.
...
// combining mark removal, whitespace normalization. This is the default pipeline.
